...
export const advancedList = async (
  model: any,
  req: NextRequest,
  populate?: any
): Promise<{ data: any[]; total: number; pageCount: number }> => {
  const {
    nextUrl: { search },
  } = req;
...
export const advancedListParams = async (
  model: any,
  searchParams: any,
  populate?: any
): Promise<{ data: any[]; total: number; pageCount: number }> => {
  ...
  for (const [key, value] of Object.entries(filteredQuery)) {
    ...
    if (match) {
      ...
      if (!queryObj[field]) queryObj[field] = {};
      if (operator === "in") {
        queryObj[field][`$${operator}`] = JSON.parse(value);
      } else if (operator === "like") {
        queryObj[field] = { $regex: value, $options: "i" };
      } else {
        queryObj[field][`$${operator}`] = value;
